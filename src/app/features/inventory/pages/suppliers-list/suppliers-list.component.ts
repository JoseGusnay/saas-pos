...
import {
  ...
  lucideBuilding2, lucidePhone, lucideMail, lucideCloudDownload,
...
import { SuppliersAdvancedFilters } from './components/suppliers-advanced-filters/suppliers-advanced-filters';
import { SupplierImportModalComponent } from '../../components/supplier-import-modal/supplier-import-modal';
  ...
  template: `
    ...
    <div class="suppliers-page">
        ...
        @if (isLoading()) {
            ...
            @if (viewMode() === 'grid') {
              <div class="data-card skeleton-card">
                <header class="data-card__header">
                  ...
                  <div class="data-card__kebab"><app-skeleton width="32px" height="32px" shape="circle"></app-skeleton></div>
                </header>
              ...
              <div class="supplier-row skeleton-row">
                <div class="row-main">
                  ...
                  <div class="supplier-info">
                    ...
                    <app-skeleton width="130px" height="0.875rem" style="margin-top: 4px"></app-skeleton>
                  </div>
                </div>
                <div class="row-status"><app-skeleton width="70px" height="22px" radius="99px"></app-skeleton></div>
                <div class="row-actions"><app-skeleton width="32px" height="32px" shape="circle"></app-skeleton></div>
              </div>
            }
          }
          ...
          @for (s of suppliers(); track s.id) {
            @if (viewMode() === 'grid') {
              ...
              <div class="supplier-row" (click)="onShowDetail(s)">
                <div class="row-main">
                ...
                <div class="row-actions" (click)="$event.stopPropagation()">
                  <app-actions-menu [actions]="supplierActions" (actionClick)="handleAction($event, s)"></app-actions-menu>
                </div>
              </div>
            }
          }
          ...
          <div class="suppliers-page__empty">
            <app-empty-state
              [icon]="activeFiltersCount() > 0 || searchQuery() ? 'lucideSearch' : 'lucideBuilding2'"
              [title]="activeFiltersCount() > 0 || searchQuery() ? 'No encontramos lo que buscas' : 'Sin proveedores registrados'"
              [description]="activeFiltersCount() > 0 || searchQuery() ? 'Intenta con otros términos o ajusta tus filtros.' : 'Agrega tu primer proveedor para empezar.'"
              [actionLabel]="activeFiltersCount() > 0 || searchQuery() ? 'Limpiar Filtros' : 'Nuevo Proveedor'"
      ...
      <app-drawer [isOpen]="isDetailOpen()" title="Detalle de Proveedor" (close)="isDetailOpen.set(false)" size="md">
        <div drawerBody>
          @if (selected()) { <app-supplier-detail [supplier]="selected()!"></app-supplier-detail> }
        </div>
        <div drawerFooter class="drawer-footer-actions">
          <app-form-button label="Historial" icon="lucideHistory" variant="ghost" type="button"
            [fullWidth]="false" (click)="onShowHistory(selected()!.id)"></app-form-button>
          <app-form-button label="Editar Proveedor" icon="lucidePencil" variant="secondary" type="button"
            [fullWidth]="false" (click)="onEdit(selected()!)"></app-form-button>
        </div>
      </app-drawer>
      ...
      <app-drawer [isOpen]="isHistoryOpen()" title="Historial de Auditoría" (close)="isHistoryOpen.set(false)">
        <div drawerBody class="history-container">
          @if (isHistoryLoading()) {
            <div class="history-loading"><app-spinner></app-spinner><span>Cargando historial...</span></div>
          } @else {
      ...
      <app-drawer [isOpen]="isFiltersOpen()" title="Filtros Avanzados" (close)="isFiltersOpen.set(false)" size="md">
        <div drawerBody>
          <app-suppliers-advanced-filters
            #advancedFilters
        ...
        <div drawerFooter class="drawer-footer-actions">
          <app-form-button label="Cancelar" variant="secondary" (click)="isFiltersOpen.set(false)"></app-form-button>
          <app-form-button label="Aplicar filtros" variant="primary" (click)="advancedFilters.applyFilters()"></app-form-button>
        </div>
      </app-drawer>
      ...
      <app-drawer
        ...
        [allowClose]="!(supplierForm?.isSubmitting())"
        (close)="onDrawerClose()"
        size="md"
      >
        <div drawerBody>
          <app-supplier-form #supplierForm (saved)="onSaved()"></app-supplier-form>
        </div>
        <div drawerFooter class="drawer-footer-actions">
          <app-form-button label="Cancelar" variant="secondary" type="button"
            [disabled]="!!(supplierForm?.isSubmitting())" (click)="onDrawerClose()"></app-form-button>
          <app-form-button
            [label]="isEditing() ? 'Actualizar Proveedor' : 'Guardar Proveedor'"
            loadingLabel="Guardando..." icon="lucideSave"
            [loading]="!!(supplierForm?.isSubmitting())"
            [disabled]="supplierForm?.supplierForm?.invalid ?? true"
      ...
      <app-modal [isOpen]="isConfirmCloseOpen()" title="Cambios sin guardar" size="sm" (close)="isConfirmCloseOpen.set(false)">
      ...
      <!-- Modal: Delete -->
      <app-modal [isOpen]="isDeleteModalOpen()" title="Confirmar Eliminación" size="sm" [allowClose]="!isDeleting()" (close)="cancelDelete()">
        <div modalBody>¿Eliminar el proveedor <strong>"{{ toDelete()?.name }}"</strong>? Esta acción no se puede deshacer.</div>
        <div modalFooter class="modal-footer-actions">
...
export class SuppliersListComponent {
  ...
  @ViewChild('advancedFilters') advancedFiltersRef!: SuppliersAdvancedFilters;
  @ViewChild('importModal') importModal!: SupplierImportModalComponent;

  supplierActions: ActionItem[] = [
  ...
  availableFields: FilterField[] = [
    ...
    { id: 'ruc', label: 'RUC/Cédula', type: 'text' },
  ...
  mappedLogs = computed<DatelineItem[]>(() =>
    this.logs().map(log => ({
      ...
      message: log.action === 'IMPORT' ? `Importación masiva de ${log.details?.count} proveedores.` : undefined,
  ...
  private readonly response = toSignal(
    toObservable(computed(() => ({
      ...
      tab: this.activeTab(), refresh: this.refreshTrigger(),
    }))).pipe(
      debounceTime(300),
      ...
      switchMap(params => {
        ...
        return this.service.findAll(filters).pipe(tap(() => this.isLoading.set(false)));
      })
    )
  );
  ...
  onFilterTreeChange(newTree: FilterNode) {
    if (newTree.type === 'group') { this.filterTree.set(newTree as FilterGroup); this.resetPagination(); }
  }
  ...
  onEdit(s: Supplier) {
    this.isDetailOpen.set(false); this.isEditing.set(true); this.isDrawerOpen.set(true);
    setTimeout(() => this.supplierForm?.setSupplier(s), 0);
  }

  onSaved() { this.isDrawerOpen.set(false); this.refreshTrigger.update(v => v + 1); this.resetPagination(); }

  onDrawerClose() {
    if (this.supplierForm?.hasUnsavedChanges()) this.isConfirmCloseOpen.set(true);
    else { this.isDrawerOpen.set(false); this.supplierForm?.resetForm(); }
  }

  forceCloseDrawer() { this.isConfirmCloseOpen.set(false); this.isDrawerOpen.set(false); this.supplierForm?.resetForm(); }
  ...
  handleAction(action: ActionItem, s: Supplier) {
    ...
    else if (action.id === 'history') this.onShowHistory(s.id);
    else if (action.id === 'delete') { this.toDelete.set(s); this.isDeleteModalOpen.set(true); }
  }

  onShowHistory(id: string) {
    this.isDetailOpen.set(false); this.isHistoryOpen.set(true); this.isHistoryLoading.set(true);
    this.service.getLogs(id).subscribe({
      next: l => { this.logs.set(l); this.isHistoryLoading.set(false); },
      error: () => { this.toastService.error('No se pudo cargar el historial'); this.isHistoryLoading.set(false); this.isHistoryOpen.set(false); },
    });
  }

  // ── Delete ────────────────────────────────────────────────────────────────────
  cancelDelete() { if (this.isDeleting()) return; this.isDeleteModalOpen.set(false); this.toDelete.set(null); }

  confirmDelete() {
    const s = this.toDelete(); if (!s || this.isDeleting()) return;
    this.isDeleting.set(true);
    this.service.remove(s.id).subscribe({
      next: () => { this.toastService.success('Proveedor eliminado correctamente'); this.refreshTrigger.update(v => v + 1); this.isDeleting.set(false); this.cancelDelete(); },
      error: (err) => { this.toastService.error(err?.error?.error ?? 'No se pudo eliminar el proveedor'); this.isDeleting.set(false); },
    });
  }
  ...
  private getLogIcon(action: string): string {
    switch (action) { case 'CREATE': return 'lucidePlusCircle'; case 'UPDATE': return 'lucideRefreshCw'; case 'DELETE': return 'lucideTrash'; case 'IMPORT': return 'lucideCloudDownload'; default: return 'lucideHistory'; }
  }

  private getLogActionLabel(action: string): string {
    switch (action) { case 'CREATE': return 'Creación'; case 'UPDATE': return 'Actualización'; case 'DELETE': return 'Eliminación'; case 'IMPORT': return 'Importación'; default: return action; }
  }
  ...
  private getChanges(oldData: any, newData: any) {
    ...
    return fields
      ...
      .map(f => ({ ...f, oldValue: this.formatValue(f.field, oldData[f.field]), newValue: this.formatValue(f.field, newData[f.field]) }));
  }
