...
@Component({
  selector: 'app-suppliers-advanced-filters',
  standalone: true,
  ...
  `,
  styles: [`.filters-body { min-height: 250px; }`]
})
export class SuppliersAdvancedFilters implements OnInit {
  ...
  ngOnInit() { this.localTree.set(structuredClone(this.filterTree())); }
  refresh() { this.localTree.set(structuredClone(this.filterTree())); }

  onLocalChange(node: FilterNode) {
    if (node.type === 'group') this.localTree.set(node as FilterGroup);
  }

  applyFilters() { this.applied.emit(this.localTree()); }
}
