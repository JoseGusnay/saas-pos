...
@Component({
  selector: 'app-presentation-import-modal',
  standalone: true,
  imports: [CommonModule, ModalComponent, SpinnerComponent, NgIconComponent],
  templateUrl: './presentation-import-modal.html',
  styleUrls: ['./presentation-import-modal.scss'],
  providers: [provideIcons({ lucideDownload, lucideUploadCloud, lucideCheck })]
})
export class PresentationImportModalComponent {
  ...
  downloadTemplate() {
    ...
    link.setAttribute('download', 'plantilla_presentaciones.csv');
  ...
  confirmImport() {
    ...
    const presentations = this.rows().map(r => ({
      name: r.name,
      country: r.country,
    ...
    this.presentationService.bulkImport(presentations).subscribe({
      next: (res) => {
        this.toastService.success(`Se importaron ${res.count} presentaciones correctamente.`);
        this.isProcessing.set(false);
        this.isOpen.set(false);
        this.imported.emit();
      },
